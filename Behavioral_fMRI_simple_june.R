...
study_raw$estimation_dError = study_raw$estimation_dError*2
...
for (i in 1:27)
{
  ...
  # {
    ...
    distNorm <- rbind(distNorm,temp2)
  # }
}

a$distNorm <- distNorm$V1
a$dMovedNorm <- distNorm$V2
a$estimationNorm <- distNorm$V3
...
rounddata_mean1 = aggregate(cbind(dMoved,TimeSpent,directionChange,nManipulation,dMovedNorm) ~ Subject + Round_fac,a,mean) 
...
round_info_withQ <- merge(study_encoding,rounddata_withQ)
...
ggplot(round_info1, aes(y=dMovedNorm, x=Round_fac, color=Round_fac)) + geom_boxplot(outlier.shape = NA) + geom_jitter(width=.1,height=0,alpha=.5,size=1)
ggplot(round_info1, aes(y=dMoved, x=Round_fac, color=Round_fac)) + geom_boxplot(outlier.shape = NA) + geom_jitter(width=.1,height=0,alpha=.5,size=1)
ggplot(round_info1, aes(y=TimeSpent, x=Round_fac, color=Round_fac)) + geom_boxplot(outlier.shape = NA) + geom_jitter(width=.1,height=0,alpha=.5,size=1)

rounddata_withQ$Condition = ifelse(rounddata_withQ$Condition == 1, 'No-body','Body')

ggplot(rounddata_withQ, aes(y=Q1z, x=Condition, color=Round_fac)) + geom_boxplot() #+ geom_jitter(width=.1,height=0,alpha=.5,size=1)
ggplot(rounddata_withQ, aes(y=Q1, x=Condition, color=Round_fac)) + geom_boxplot() #+ geom_jitter(width=.1,height=0,alpha=.5,size=1)
...
indivdata = aggregate(cbind(dError,distNorm,dMoved,dMovedNorm,TimeSpent,directionChange,nManipulation,totalAngle,estimation_dError,estimationNorm,ErrorMetaCog) ~ Condition + Subject,a,median)

test <- indivdataQ %>% group_by(Subject) %>% mutate(Body_Effect = (distNorm[Condition=='No-body'] - distNorm[Condition=='Body']))
indivdataWithoutCon = aggregate(cbind(dError,Body_Effect,Q1,Q1z,Q2,Q2z) ~ Subject,test,median)

ggplot(indivdataWithoutCon, aes(y=dError, x=Q2z)) + geom_point(alpha=.7,size=2) + geom_smooth(method="lm", se = FALSE, fill = "NA")
cor.test(~dError + Q2z, data = indivdataWithoutCon, method ="spearman", continuity = TRUE, conf.level = 0.95)


indivdataQ = aggregate(cbind(dError,distNorm,dMoved,dMovedNorm,TimeSpent,directionChange,nManipulation,totalAngle,estimation_dError,estimationNorm,ErrorMetaCog,Q1,Q2,Q3,Q4,Q1z,Q2z,Q3z,Q4z) ~ Condition + Subject,a,median)
indivdata_mean = aggregate(cbind(dError,distNorm,dMoved,dMovedNorm,TimeSpent,directionChange,nManipulation,totalAngle, estimation_dError,estimationNorm,ErrorMetaCog) ~ Condition + Subject,a,mean)
indivdata_meanQ = aggregate(cbind(dError,distNorm,dMoved,dMovedNorm,TimeSpent,directionChange,nManipulation,totalAngle, estimation_dError,estimationNorm,ErrorMetaCog,Q1,Q2,Q3,Q4,Q1z,Q2z,Q3z,Q4z) ~ Condition + Subject,a,mean)

a2 <- subset(a, Index != 1)
indivdata_wo_1st = aggregate(cbind(dError,distNorm,dMoved,dMovedNorm,TimeSpent,directionChange,nManipulation,estimation_dError,estimationNorm,ErrorMetaCog) ~ Condition + Subject + Group,a2,median)
indivdata_mean_wo_1st = aggregate(cbind(dError,distNorm,dMoved,dMovedNorm,TimeSpent,directionChange,nManipulation,estimation_dError,estimationNorm,ErrorMetaCog) ~ Condition + Subject + Group,a2,mean)


a$Performance = a$dError / (a$TimeSpent + 10)
indivdata_test = aggregate(cbind(dError,distNorm,dMoved,TimeSpent,directionChange,nManipulation,estimation_dError,estimationNorm,ErrorMetaCog,Performance) ~ Condition + Index,a,median)
indivdata_mean_test = aggregate(cbind(dError,distNorm,dMoved,TimeSpent,directionChange,nManipulation,estimation_dError,estimationNorm,ErrorMetaCog,Performance) ~ Condition + Index,a,mean)
...
ggplot(a, aes(y=dError, x=Index2, color=Index)) + geom_boxplot() + geom_point()
ggplot(a, aes(y=distNorm, x=Index2, color=Index)) + geom_boxplot() + geom_point()
...
ggplot(a, aes(y=dMovedNorm, x=Condition,color=Condition)) + geom_boxplot() 
# ggplot(a, aes(y=ErrorMetaCog, x=Condition,color=Condition)) + geom_boxplot(outlier.shape = NA) 
...
test <- indivdata %>% group_by(Subject) %>% mutate(Body_Effect = (distNorm[Condition=='No-body'] - distNorm[Condition=='Body']))
test <- indivdata_mean %>% group_by(Subject) %>% mutate(Body_Effect = (distNorm[Condition=='No-body'] - distNorm[Condition=='Body']))
p <- ggplot(test, aes(y=distNorm, x=Condition, group=Subject, color=Condition)) +  geom_line(aes(color=Body_Effect>0),size = 0.8) + geom_point(size = 2.5) + scale_color_manual(values=c("#F8766D", "#00BFC4", "#00BFC4", "#F8766D")) #+ geom_point(aes(color=Condition), size=2.5)
...
test <- indivdata_mean %>% group_by(Subject) %>% mutate(Body_Effect = (TimeSpent[Condition=='No-body'] - TimeSpent[Condition=='Body']))
test <- indivdata_mean %>% group_by(Subject) %>% mutate(Body_Effect = (dMovedNorm[Condition=='No-body'] - dMovedNorm[Condition=='Body']))
p <- ggplot(test, aes(y=dMovedNorm, x=Condition, group=Subject, color=Condition)) +  geom_line(aes(color=Body_Effect>0),size = 0.8) + geom_point(size = 2.5) + scale_color_manual(values=c("#F8766D", "#00BFC4", "#00BFC4", "#F8766D")) #+ geom_point(aes(color=Condition), size=2.5)
...
ggplot(indivdata, aes(y=distNorm, x=Condition,color=Condition, group =Subject)) +  geom_line(color="black") + geom_point(size=2.0) 
ggplot(indivdata_mean, aes(y=distNorm, x=Condition,color=Condition, group =Subject)) +  geom_line(color="black") + geom_point() 
ggplot(indivdata_mean, aes(y=dError, x=Condition,color=Condition, group =Subject)) +  geom_line(color="black") + geom_point() 

ggplot(indivdata_mean, aes(y=dMoved, x=Condition,color=Condition, group =Subject)) +  geom_line(color="black") + geom_point() 

ggplot(indivdata, aes(y=dMoved, x=Condition,color=Condition)) + geom_boxplot() #+ facet_wrap(~Subject)
ggplot(indivdata, aes(y=TimeSpent, x=Condition,color=Condition)) + geom_boxplot() #+ facet_wrap(~Subject)
...
ggplot(indiv_mean_sd, aes(y=dMovedNorm_mean, x=Condition,fill=Condition)) + geom_bar(stat="identity",width=.6) + geom_errorbar(aes(ymin=dMovedNorm_mean-dMovedNorm_se,ymax=dMovedNorm_mean+dMovedNorm_se), size = .3, width=.2,position=position_dodge(.9)) # + coord_cartesian(ylim=c(75,90))
...
gamma <- fitdistr(a$dError, "gamma")
qqp(a$dError, "gamma", shape = gamma$estimate[[1]], rate = gamma$estimate[[2]])


a_test <- subset(a, a$Index != 1)
ggplot(a, aes(x=dError,y=..density..)) + geom_density() + facet_wrap(~Subject,scales='free')
ggplot(a, aes(x=distNorm,y=..density..)) + geom_density() #+ facet_wrap(~Subject)

shapiro.test(a$dError)
shapiro.test(a$distNorm)
...
ggplot(a, aes(x=,y=..density.., group=Condition,  color=Condition)) + geom_density()

ggplot(a, aes(x=estimation_dError,y=..density.., group=Condition,  color=Condition)) + geom_density()
ggplot(a, aes(x=estimationNorm,y=..density.., group=Condition,  color=Condition)) + geom_density()
...
ggplot(a_test, aes(x=dMovedNorm,y=..density.., group=Condition,  color=Condition)) + geom_density()
...
ggplot(indivdata, aes(y=distNorm, x=Condition,color=Condition)) + geom_boxplot(outlier.shape = NA) + geom_jitter(width=.05,height=0,alpha=.7,size=1.5) #+ facet_wrap(~Subject)
...
ggplot(indivdata_mean, aes(y=estimationNorm, x=Condition,color=Condition)) + geom_boxplot(outlier.shape = NA) #+ facet_wrap(~Subject)
...
sub_a = subset(a, Index != 1 & Index != 14 )
sub_a = a
...
# m1 = glmer(distNorm ~ Condition  + (1|Subject),a, family=Gamma(link=log))
# m2 = glmer(distNorm ~ Condition  + (Condition|Subject),a,family=Gamma(link=log))
...
m1 = lmer(dMovedNorm ~ Condition  + (1|Subject),a_test)
m2 = lmer(dMovedNorm ~ Condition  + (Condition|Subject),a_test)
# 
anova(m1,m2)
...
rounddata1$Condition = ifelse(rounddata1$Condition == 1, 'No-body','Body')
sub_indivdata = indivdata_mean
...
wilcox.test(sub_indivdata$dMovedNorm[sub_indivdata$Condition=='Body'],sub_indivdata$dMovedNorm[sub_indivdata$Condition=='No-body'],paired=T)
...
am1 <- aov(distNorm ~ Condition + Subject, data=indivdata_mean)
summary(am1)

am2 <- aov(distNorm ~ Condition + Error(Subject/Condition), data=indivdata_mean)
summary(am2)
...
t.test(indivdata_mean$distNorm[indivdata_mean$Condition == 'Body'],indivdata_mean$distNorm[indivdata_mean$Condition == 'No-body'] , paired = T)
...
t.test(indivdata$distNorm[indivdata$Condition == 'Body'],indivdata$distNorm[indivdata$Condition == 'No-body'] , paired = T)
...
## Questionnaire
sub_a <- subset(a, Subject != 1 & Subject != 26)
indivQ = aggregate(cbind(Q1z,Q2z,Q3z,Q4z) ~ Condition + Subject,sub_a,mean)
...
temp_round <- subset(rounddata_withQ, Subject != 26) 
m1 = lmer(Q2z ~ Condition + (1|Subject),temp_round)
m2 = lmer(Q2z ~ Condition + (Condition|Subject),temp_round)
anova(m1,m2)
summary(m1)
anova(m1)

temp_indiv <- subset(indivdataQ) #& Subject != 21 & Subject != 22 & Subject != 23 & Subject != 24 & Subject != 25 & Subject != 27

m1 = lmer(Q1z ~ Condition + (1|Subject),temp_indiv)
summary(m1)
anova(m1)
